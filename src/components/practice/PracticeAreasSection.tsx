...
export const PracticeAreasSection: React.FC<PracticeAreasProps> = ({
  ...
  return (
    <section id="practice-areas" className="py-20 px-4 max-w-7xl mx-auto border-t border-[#9C7A58]/20">
        ...
        {practiceAreas.map((area) => {
          ...
          return (
            <button
              ...
              className={`px-5 py-2.5 text-xs md:text-sm font-medium rounded-sm transition-all cursor-pointer ${
                isActive
      ...
      {selectedArea && (
        <div className="bg-white border border-[#9C7A58]/20 p-6 md:p-10 rounded-sm shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {selectedArea.detailedProfile.services.map((cat, idx) => (
              <div key={idx} className="bg-[#FAF6F0]/50 p-6 rounded-sm border border-[#9C7A58]/10">
                ...
                <ul className="space-y-2.5 text-xs md:text-sm text-[#6B6B6B]">
